...
def run(params, absFilePath):
	...
	folder = base + date+ "_Daniel_pHContr\\"  #_Daniel_pHChange
	...
	img=LoadImagFiles.Load(file, folder_correct)#
	S = np.shape(img)
	...
	plt.figure(2)
	ax = plt.subplot(1, 1, 1)
	plot_actual_predict(ax, Y, y_pred, DATA_TOT[1,::data_sub])
	for ii in range(2):
		for tick in ax.xaxis.get_major_ticks():
			tick.label1.set_fontsize(fontsize)
		for tick in ax.yaxis.get_major_ticks():
...
def export_predictions(Y_exp, Y_pred):
    ...
    np.savetxt("Exp_Pred_Ratio.txt", DAT, delimiter=',')#, fmt='%d'
